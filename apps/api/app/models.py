...
class ReplyAppJobSource(str, enum.Enum):
    MENTION = "mention"
...
class ReplyAppJobStatus(str, enum.Enum):
    DRAFT = "draft"  # opportunity scored, not yet generating
    QUEUED = "queued"
    RUNNING = "running"
    GENERATED = "generated"  # app ready, not yet posted
    REPLIED = "replied"
    SKIPPED = "skipped"
    FAILED = "failed"
...
class ReplyApp(Base):
    """Hosted mini-app artifact generated by Grok."""

    __tablename__ = "reply_apps"
    ...
    prompt_brief: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    generation_metadata: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB, nullable=True)
    preview_image_path: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    jobs: Mapped[list["ReplyAppJob"]] = relationship(back_populates="app")


class ReplyAppJob(Base):
    """One mention / opportunity / manual request through the reply-app pipeline."""

    __tablename__ = "reply_app_jobs"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=_uuid)
    source: Mapped[ReplyAppJobSource] = mapped_column(
        Enum(ReplyAppJobSource, name="reply_app_job_source"),
        default=ReplyAppJobSource.MANUAL,
    )
    status: Mapped[ReplyAppJobStatus] = mapped_column(
        Enum(ReplyAppJobStatus, name="reply_app_job_status"),
        default=ReplyAppJobStatus.QUEUED,
        index=True,
    )
    # Source tweet (the one we reply under)
    source_tweet_id: Mapped[Optional[str]] = mapped_column(String(64), unique=True, nullable=True, index=True)
    source_tweet_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    source_author_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    source_author_username: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    conversation_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    mention_tweet_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)

    intent_json: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB, nullable=True)
    ...
    app_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        ForeignKey("reply_apps.id", ondelete="SET NULL"), nullable=True
    )
    reply_tweet_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    reply_tweet_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)

    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    started_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    finished_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)

    app: Mapped[Optional[ReplyApp]] = relationship(back_populates="jobs")
